...
- LLM: dedicated module worker at `site/triage-llm-worker.js` lazy-loads `@huggingface/transformers@3.5.0` from jsDelivr and instantiates `AutoTokenizer` + `AutoModelForCausalLM` against `onnx-community/gemma-4-e2b-it-ONNX` with `dtype:'q4f16'`, `device:'webgpu'`. Generation uses `TextStreamer` + `InterruptableStoppingCriteria` (recipe lifted from `huggingface/transformers.js-examples/llama-3.2-webgpu/src/worker.js`). The worker postMessages `{status:'update',output}` per decoded chunk; the main thread appends each chunk to a live assistant message bubble, then dispatches tool calls on `{status:'complete'}`. A second `send` while generating fires `{type:'interrupt'}`. Tool calls (`add_card`, `remove_card`, `highlight_card`, `clear_screen`) parsed from fenced ` ```tool ` blocks; each turn rebuilds a fresh system prompt from the active scenario + current scratchpad — chat history pruned to 1 user + 1 assistant message after every turn. Fallback: if WebGPU is unavailable or the worker errors, `send()` routes through `simulateAssistant` and prefixes the reply with `(error) … — falling back to simulate.`
