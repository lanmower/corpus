...
- **Theming**: `site/theme.js` owns `data-theme` on `<html>` (`light`/`dark`/`auto`/`contrast`, key `corpus.theme.v1`); `.ds-247420` is on `<html>` so `.ds-247420[data-theme=...]` semantic-token blocks theme the whole app. Verified-correct + the SDK-clobber-guard and raw-palette pitfall: see rs-learn (recall "corpus light/dark theme SDK clobber").
