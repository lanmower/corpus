...
## Quests/badges/notes removal (2026-05-06)

Trimmed the gamification surface. `site/quests.js`, `site/badges.js`, `site/notes.js` deleted. Routes `#quests`, `#badges`, `#notes` aliased to `today` via `ROUTE_ALIASES` (old bookmarks resolve to today, no 404). The today-page quest panel + recent-achievements panel are gone; `renderQuestsPanel`, `questRow`, `renderAchievementsPanel`, `renderQuests`, `renderBadges`, `renderNotes`, `handleHighlightOrNote`, and `runBadgeEvaluation` removed from `app.js`. `h`/`n` keys are inert (also dropped from the shortcuts modal); `g n` removed. More-menu now lists `cases · calendar · stats · mistakes · settings`.

Kept: `game.js` XP/level system (xp bar, awardXP from card grades + section ticks + pomodoros + cases), `confetti.js`, `toast.js`, `#mistakes` + `mistakes.js`. `game.awardBadge` + `g.badges`/`g.unlocks` storage fields persist on `corpus.game.v1` because awardBadge is still exported on `window.__game` — nothing in app.js calls it any more, so badges accrue zero entries; left as-is to avoid migrating user state. localStorage keys `corpus.quests.v1`, `corpus.badges.*`, `corpus.notes.v1` are no longer read or written; existing user data is untouched.

`test.js` 12/12 green. Browser-witnessed at `http://127.0.0.1:8765/`: topbar = `today guides review more tutor`, more-menu has no quests/badges/notes; `#today` renders without quest/achievements panels; `#quests`/`#badges`/`#notes` all set document.title to "today · corpus"; grading a card moves `window.__game.load().xp` from 30 → 50; zero pageerrors / console errors.

## Gamification fully stripped + guide TOC overhaul (2026-05-07)

Followed up the quests/badges/notes excision by removing the remaining XP/level/confetti machinery — the site now reads as a study notebook with zero game-currency framing. Deleted `site/game.js` and `site/confetti.js`. Pulled every `awardXP`, `renderXpChip`, `renderXpBarFull`, `awardCardXP`, `xp-chip` topbar mount, `pomodoro:done` XP listener, `case:graded` XP handler, and the `gamification` settings panel (suppress-toasts toggle + reset-XP button) out of `site/app.js`. CSS purged of `--c-xp` / `--c-quest` tokens, `.xp-chip` / `.xp-bar-full` / `.quest-*` / `.badge-*` / `.achievements-panel` / `.toast-xp` / `.toast-level` / `.confetti-canvas` blocks. `corpus.game.v1` is no longer read or written anywhere; existing storage is left untouched. `'game:xp'` window event handler removed. SW cache key remains `__BUILD_VERSION__`-driven (per-deploy auto-version) so no manual bump needed; `?v=` cache busters likewise auto-version on Pages deploy. SHELL list in `sw.js` was already minimal (only `./`, `./index.html`, `./manifest.webmanifest`) so nothing to remove there.

Today status line (`renderStatusLine`) reframed: was `day N · M due · streak K · goal X/Y`, now `<YYYY-MM-DD> · M due · X reviewed today`. Streak / goal / day-counter dropped from the chrome — `progress.js` still tracks streak + goal internally for the recap/forecast and the cram-day banner, but no chip surfaces those as game numbers on `#today`.

Subject deepdive (`#subject/<name>`) sidebar TOC rebuilt for the long guides:
- New `buildGuideToc(subj, shard, ticks)` — replaces the flat 50-section list with collapsible H2 groups, each H2 a `<details open>` whose summary shows the H2 title + an `N/M` mono progress count (children ticked / total children).
- Search-within-guide filter input at the top of the panel — typing live-filters TOC rows by `data-title` substring; non-matching groups collapse out, matching groups force-open. `applyTocFilter(panel, query)` is the handler.
- TOC rows are anchor links (`<a class="toc-link" href="#g-<slug>-<line>">`) that smooth-scroll the matching `<h2>`/`<h3>` in the rendered guide body. The anchor ids match those `renderMarkdown` already emits (`g-${slug(title)}-${lineIdx}`).
- Per-row tick checkbox preserved (still writes `corpus.guide.v1`); rows get `.done` strike-through when ticked.
- Panel head shows a top-level `tickedTotal/totalSections` summary.
- New floating `back-to-top` button (`mountBackToTop()`) — fixed bottom-right, fades in once `window.scrollY > 400`, smooth-scrolls to top on click, removes/rebinds its scroll listener on each subject render.

CSS additions: `.toc-panel`, `.toc-filter`, `.toc-groups`, `.toc-group`, `.toc-h2-summary` (with `▸` rotation indicator on `[open]`), `.toc-h2-progress`, `.toc-row.h2/.h3`, `.back-to-top` (with `.hidden` fade-out state). All within existing design tokens.

Test gate (`test.js`) updated: dropped XP-curve / award-xp / suppress-toasts / level-rise assertions; added asserts that `site/game.js` and `site/confetti.js` are absent on disk, `app.js` has no `game.`/`confetti.`/`awardXP`/`renderXpChip`/`xp-chip` references, `style.css` has no `--c-xp`/`.xp-chip`/`.confetti-canvas`, and `app.js` contains the new `buildGuideToc`/`toc-filter`/`toc-h2-progress`/`mountBackToTop`/`back-to-top`/`applyTocFilter` symbols. Status-line shape asserts now check for `${due} due` + `reviewed today` and explicitly forbid the old `day`/`streak`/`goal` template strings. `node test.js` → 16/16 green.
